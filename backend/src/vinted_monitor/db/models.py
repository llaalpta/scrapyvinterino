...
class UserSession(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "(user_id IS NULL AND authenticated_at IS NULL) OR "
            "(user_id IS NOT NULL AND authenticated_at IS NOT NULL)",
...
class VintedSession(Base):
    ...
    impersonate: Mapped[str] = mapped_column(String(40))
    country_code: Mapped[str] = mapped_column(String(2))
    locale: Mapped[str] = mapped_column(String(20))
    accept_language: Mapped[str] = mapped_column(String(120))
    viewport_size: Mapped[str] = mapped_column(String(40), default="1920x1080")
    vinted_screen: Mapped[str] = mapped_column(String(40), default="catalog")
...
class RunEventOutbox(Base):
    ...
    __table_args__ = (Index("ix_run_event_outbox_created_at_event_id", "created_at", "event_id"),)

    event_id: Mapped[int] = mapped_column(ForeignKey("run_events.id", ondelete="CASCADE"), primary_key=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
