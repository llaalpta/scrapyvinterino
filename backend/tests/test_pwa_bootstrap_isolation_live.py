            ...
            for surface, failed_path in FAILED_SURFACES:
                ...
                def guard(route: Route, *, expected_surface: str = surface, expected_path: str = failed_path) -> None:
                    ...
                    if (
                        ...
                        and parsed.path != "/api/auth/login"
                    ):
                ...
                try:
                    ...
                    expected_message = (
                        f"Carga inicial incompleta: {surface}. "
                        "Las demas secciones disponibles siguen operativas."
                    )
                    ...
                    expect(monitor_selector).to_be_visible()
                    expect(monitor_selector.locator(f'option[value="{scenario.source_id}"]')).to_have_text(
                        scenario.source_name
                    )

                    page.get_by_role("button", name="Monitores", exact=True).click()
                    expect(
                        page.get_by_role("button", name=f"{scenario.source_name}, inactivo", exact=True)
                    ).to_be_visible()
                finally:
                    page.close()
...
def _snapshot(source: SearchSource) -> dict[str, object]:
    return {
        ...
        for column in SearchSource.__table__.columns
    }
...
def _assert_no_runtime_graph(source_id: int) -> None:
    with SessionLocal() as db:
        ...
        assert db.scalar(
            select(func.count()).select_from(MonitorSession).where(MonitorSession.source_id == source_id)
        ) == 0
        ...
        assert db.scalar(
            select(func.count()).select_from(VintedSession).where(VintedSession.source_id == source_id)
        ) == 0
