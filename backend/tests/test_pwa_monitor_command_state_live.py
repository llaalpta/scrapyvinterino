...
def _exercise_live_stack(scenario: Scenario, *, api_url: str, pwa_url: str) -> int:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            prepare_button = page.get_by_role("button", name="Preparar sesion", exact=True)
            start_button = page.get_by_role("button", name="Iniciar sesion", exact=True)
            detail_ref = page.get_by_label("ID o URL de item para probar detalle", exact=True)
            detail_probe = page.get_by_role("button", name="Probar detalle", exact=True)
            expect(archive_button).to_be_enabled()
            expect(prepare_button).to_be_enabled()
            expect(start_button).to_be_enabled()
            detail_ref.fill("123456")
            expect(detail_probe).to_be_enabled()
            ...
            expect(prepare_button).to_be_disabled()
            expect(start_button).to_be_disabled()
            expect(detail_ref).to_be_disabled()
            expect(detail_probe).to_be_disabled()
