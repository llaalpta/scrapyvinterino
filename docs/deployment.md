...
El catalogo no tiene un contador de retries configurable ni reintentos HTTP internos. Antes de aceptar candidatos, cada perfil puede usar el intento inicial y un unico sticky nuevo; el reemplazo exige el diagnostico proxificado y no llama a Vinted si observa la IP ya rechazada. Si ambos fallan, ese perfil entra una vez en cooldown y el mismo run considera una vez los restantes perfiles elegibles en orden normal, con admision/capacidad serializada. No se llama a una API de rotacion ni se repite el pool: si ninguno sirve, termina como `session_acquisition_exhausted`. La PWA permite entonces una orden explicita contra uno de los perfiles enfriados que participo en ese ultimo inicio: omite solo su cooldown para un sticky/diagnostico nuevos, no recorre otros perfiles y no limpia la penalizacion antes del resultado.
...
- La convergencia operativa 14.35 del 2026-07-15 recreo solo la API y aplico `0018 -> 0019` sobre el volumen real con worker/watchdog detenidos y Redis vacio. El upgrade retiro exactamente nueve `vinted_sessions` incompatibles; los fingerprints de monitores, configuracion, items, runs, sesiones de monitor, oportunidades, eventos, publicaciones, proxy legacy y auth quedaron identicos. Tras limpiar el usuario QA, auth, monitores, proxys, scheduler, oportunidades y runs devolvieron `200`, sin `500`, trafico Vinted/proxy ni residuos.
