...
- Recurring activation selects available egress before persisting active/session/deadline state and compensates activation if the initial run cannot be created. A second activation is rejected. The producer rechecks `next_run_at` after locking and commits window deferrals before cache or egress work can roll them back.
