...
- Manual start and `Ejecutar ahora` remain synchronous from the API and use the same provider stack. Start owns a zero-opportunity baseline before opening the session; later runs require and reuse that active session.
...
- Manual PWA views hide the temporary explicit recurring baseline surface. While stopped they expose `Iniciar sesion`; while active they expose one `Ejecutar ahora` and stop. Configuration, archive and conflicting commands are disabled during a local command or loaded non-terminal run. Stop returns `409` for `running/finalizing` until 14.34.3 owns graceful drain.
