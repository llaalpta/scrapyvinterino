...
- The monitor event stream now starts at the database tail when no cursor is supplied, honors explicit query cursor over `Last-Event-ID`, announces its cursor/retry interval, drains backlog without poll pauses, heartbeats every 15 seconds, disables intermediary transformation/buffering and preserves detail redaction. Focused stream/redaction tests passed (`12 passed`).
- The dashboard controller owns one `EventSource` only while Monitors is open. REST history and live events merge by ID; terminal events debounce source-specific run history and stats refreshes, and reload opportunities only for positive or unavailable `opportunities_created`. The dormant per-run viewer no longer opens another stream.
- Recurring activation persists its first post-launch deadline before the immediate run. Tests cover exactly one immediate run, the effective 60-66 second range for interval 60/jitter 10%, and PostgreSQL deadline precedence over a stale in-process scheduler value (`4 passed`).
- The complete local gate passed Ruff, all `383` backend tests, PWA ESLint/build, API health and frontend HTTP smoke. The worker stayed stopped during browser QA and no Vinted/proxy request was made.
- Playwright against the restarted live PWA held exactly one SSE for 15 idle seconds with no repeated REST requests. A synthetic event rendered once in `508 ms`; a zero-opportunity terminal caused one source, monitor-run and monitor-stats refresh and no opportunity/global-run refresh; leaving closed the stream and returning resumed with `last_event_id=21638`, rendering the intervening event once. Tail following and the new-event control returned to the bottom on desktop and 390 px. All six synthetic QA events were deleted afterward.
