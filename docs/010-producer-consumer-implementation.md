...
- Manual PWA views expose `Iniciar sesion` while stopped and one `Ejecutar ahora` plus stop while active. Configuration, archive and conflicting commands are disabled during a local command or loaded non-terminal run. Stop returns `409` for `running/finalizing` until 14.34.3 owns graceful drain.
