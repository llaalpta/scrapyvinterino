...
- Every `open`, `error`, `stream_ready` and `monitor_event` callback now verifies that the `EventSource` which registered it is still current. The current failure closes and clears only its own instance before scheduling at most one reconnect timer; callbacks from replaced or closed instances are inert.
