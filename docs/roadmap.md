...
| 14.3 | not-started | Service lifecycle and health decision tree | `docs/architecture.md`, `docs/deployment.md` | Trace Compose startup, migrations, API, worker producer/consumers, health, shutdown, restart and operator recovery against the real containers. Document discrepancies as later fixes. |
| 14.4 | not-started | Monitor, session and run decision tree | `docs/architecture.md`, owning feature specs | Trace PWA/API launch, prepared-session selection, preparation/refresh/invalidation, manual/recurring runs, cadence and terminal states with a bounded real monitor run. |
| 14.5 | not-started | Queue, concurrency and cache decision tree | `docs/architecture.md`, `docs/data-model.md`, `docs/specs/008-scheduler.md` | Trace admission, ready/reserved/processing/ACK/retry/dead-letter, locks, capacity, Redis seen/detail state and recovery using the real worker and Redis. |
| 14.6 | not-started | Persistence, events and logging decision tree | `docs/architecture.md`, `docs/data-model.md`, `docs/deployment.md`, `docs/security.md` | Trace transaction boundaries, run/item/opportunity persistence, outbox/SSE publication, process logs, redaction, retention and PWA refresh from real run evidence. |
| 14.7 | not-started | Documentation drift and legacy removal gate | Docs changed by 14.3-14.6 and their directly affected code | Compare only the documented flows from 14.3-14.6 with their captured runtime evidence, remove confirmed stale passages, and turn unrelated discrepancies into contained roadmap tasks. |
