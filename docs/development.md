...
El comando predeterminado migra dos bases nuevas y ejecuta dos veces el canary real de identidad. `-Scenario` solo admite los nombres cerrados de `ValidateSet`; nunca acepta un target arbitrario. `same-profile-recovery` levanta un ASGI controlado, Vite y Playwright, atraviesa un proxy HTTP loopback para el diagnostico forzado y prueba otra IP aceptada y la misma IP rechazada sin construir un segundo provider Vinted. En el mismo ciclo ejecuta los casos worker/Redis de exito, agotamiento, transporte, `429` y no-replay tras candidatos.

`proxy-only-regression` ejecuta todos los runs manuales con destinos externos bloqueados; `proxy-cooldown` cubre configuracion, migracion, fallos clasificados, fences y admision; `proxy-sticky-contract` recorre alta/edicion del contrato e invalidacion/reemplazo de contexto. Los escenarios PWA usan API `8001`, Vite `5176`, Playwright y proveedores controlados solo en la frontera externa; no añaden endpoints, flags ni fallbacks de produccion. Recurrente y session-stop recorren ademas scheduler, Redis y consumer dentro del test. Worker-redis usa imagenes locales, una red `--internal` y recursos desechables. Full conserva las URLs contractuales para unitarios falsos y ejecuta aparte los modulos que exigen loopback.

`-Repeat` se limita a `1..3`. El runner no arranca servicios Compose, no carga la `.env` raiz y bloquea egress heredado mediante proxys de entorno a loopback. Una lease reserva Redis 15; el cleanup solo hace `FLUSHDB` si conserva esa lease y elimina exclusivamente rol/base, procesos y ficheros QA propios. Antes y despues compara fingerprints opacos de PostgreSQL operativo y Redis 0; una diferencia falla de forma visible y nunca restaura datos automaticamente.
