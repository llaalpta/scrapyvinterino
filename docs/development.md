...
El comando predeterminado migra dos bases nuevas y ejecuta dos veces el canary real de identidad. `-Scenario` traduce `identity`, `catalog-fail-stop`, `prepared-session-read-model`, `manual-session-start-baseline`, `recurring-session-start-baseline`, `worker-redis-availability` y `full` a targets cerrados; nunca acepta un target arbitrario. Prepared-session y los dos escenarios de session-start usan API/Vite/Playwright reales, bloquean hosts no loopback y cierran solo sus procesos. Los escenarios manual y recurrente arrancan un ASGI exclusivo de tests que inyecta por fichero temporal un proveedor controlado solo en la frontera Vinted; no añaden endpoint, flag ni fallback de produccion. El recurrente mantiene worker y watchdog operativos detenidos, pero recorre el `SchedulerRunner`, la cola Redis y el `TaskConsumer` reales dentro del proceso de prueba, con una clave de cola exclusiva, y limpia todo el grafo SQL/Redis al terminar. Worker-redis usa imagenes locales, una red `--internal`, ownership por token y una base/Redis desechables; desactiva el restart antes de retirar el worker y restaura exactamente las redes del PostgreSQL operativo. Full conserva las URLs contractuales para unitarios falsos y ejecuta aparte, con destinos loopback, el modulo que lo exige; los dos grupos comparten la misma base efimera. `-Repeat` se limita a `1..3`. El runner localiza los contenedores sin arrancar servicios Compose ni cargar la `.env` raiz, sanea el entorno heredado y bloquea egress mediante proxys de entorno a loopback. Una lease reserva Redis 15; el cleanup solo hace `FLUSHDB` si conserva esa lease y elimina exclusivamente el rol/base generados. Antes y despues compara fingerprints sin valores visibles de PostgreSQL operativo y Redis 0; una diferencia falla de forma visible y nunca intenta restaurar datos automaticamente.
