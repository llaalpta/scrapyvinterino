...
#### Diagnostico real desde Test IP

El siguiente gate usa exactamente una pulsacion de `Test IP` en la PWA viva con un usuario local QA temporal. El navegador solo accede a loopback; API, PostgreSQL, Redis y el Vite existente no se reinician, mientras worker/watchdog permanecen detenidos y no existe trabajo activo. La accion emite un unico `POST /api/proxy-profiles/{id}/test`; el backend inicia un solo `GET https://api.ipify.org?format=json` mediante el proxy configurado, con timeout de `10` segundos y sin retry de aplicacion. Los redirects fisicos que permita la libreria son el unico residual de transporte. No se llama a Vinted, DataDome ni otro destino.

La PWA debe mostrar primero el estado pendiente y despues el resultado terminal que devuelve el API. Un resultado positivo exige `last_test_status=success` y una IP presente; el HTTP `200` local o un success sin IP no bastan. La evidencia conserva solo status seguro, duracion, presencia de IP y clasificacion redactada del error; no imprime IP, payload, URL del proxy, username, password ni excepcion cruda. Comprueba que el DOM usa el username enmascarado y que ni el password ni la URL autenticada aparecen en DOM/error; el username crudo que aun incluye `ProxyProfileRead` permanece como limitacion ya inventariada en 14.12.8 y no se reproduce en la evidencia. La fila del proxy conserva `last_test_*` como telemetria operativa legitima del propio comando; no se falsea ni restaura. Un success puede limpiar `failure_count`/cooldown como comportamiento propio del comando; un fallo no debe cambiarlos. En ningun caso se repite ni se modifican configuracion o credenciales.

`Test IP` usa la identidad base, no el username sticky de los monitores. Por tanto un success solo demuestra autenticacion/gateway HTTPS general; no prueba el pais, `ipwho.is`, DataDome, Vinted ni la identidad sticky y no desbloquea por si solo 14.38.

El cleanup cierra Playwright y elimina por ID el usuario QA, su preautenticacion y sus sesiones autenticadas. Debe devolver a cero las filas QA, Redis, fuentes activas, runs no terminales y sesiones abiertas, manteniendo el item previo, el scheduler ausente, los ejecutores detenidos y los servicios/puertos inicialmente operativos.

El gate de 2026-07-16 completo una unica pulsacion real. La PWA mostro `Probando...` con el control deshabilitado y despues el terminal; Playwright observo exactamente un POST, cero mutaciones adicionales y ningun intento no-loopback del navegador, mientras el log API registro una sola finalizacion. El comando termino en `4.409 s` con `last_test_status=failed`, sin IP y con error redactado clasificado `proxy_connect`. Password y URL autenticada no aparecieron en el error/DOM; el DOM uso el username enmascarado y la respuesta mantuvo el legacy de username crudo sin imprimirlo.

PostgreSQL conservo `failure_count=1`, cooldown, identidad/configuracion/credenciales cifradas, `last_used_at` y cero Vinted sessions; solo `last_test_*`/`updated_at` representan el comando real. Los dos arranques locales fallidos del harness anteriores no alcanzaron el POST ni trafico externo. El cleanup elimino los tres usuarios temporales, dos sesiones autenticadas y una preautenticacion revocada; Redis, filas QA, fuentes activas, runs no terminales y sesiones abiertas quedaron a cero, con el item previo y servicios iniciales intactos. Este resultado demuestra que tambien falla la conectividad de la identidad base hacia el eco HTTPS; no identifica por si solo si la causa final es gateway, cuenta, autenticacion o red del proveedor.

Un reintento posterior solo se admite tras observar que el contenedor API vuelve a resolver el hostname del gateway. Conserva el mismo perfil y contrato, permite una unica pulsacion PWA y un unico GET logico a ipify, y exige success con IP presente; cualquier nuevo fallo termina sin otro endpoint, retry o cambio de credenciales. El cleanup y la telemetria legitima siguen las mismas reglas anteriores.

El reintento acotado de 2026-07-16 partio con el gateway resolviendo dentro del contenedor API y completo una sola pulsacion PWA en `1.984 s`. La interfaz mostro el pendiente con el control deshabilitado y el terminal; el unico POST finalizo con `last_test_status=success`, IP presente y sin error. Playwright no observo mutaciones adicionales ni trafico no-loopback del navegador, y el log API registro una sola finalizacion. PostgreSQL limpio legitimamente `failure_count` y cooldown, mantuvo cero Vinted sessions y no se editaron la configuracion ni las credenciales del perfil.

El cleanup elimino un usuario QA, su sesion autenticada y su preautenticacion revocada. Redis, filas QA, fuentes activas, runs no terminales y sesiones abiertas quedaron a cero; el item previo, el scheduler ausente, Vite y los servicios iniciales se conservaron, con worker/watchdog detenidos y sin Chrome headless residual. Este success retiro el bloqueo de conectividad base para repetir 14.38, pero no demostro pais, identidad sticky, DataDome ni Vinted: esas fronteras se validaron despues en el gate real sin convertir este diagnostico en un fallback o retry de runtime.
