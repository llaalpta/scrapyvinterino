...
Al montar el dashboard autenticado, las lecturas iniciales de monitores, oportunidades, runs y proxys se resuelven y aplican de forma independiente. Una respuesta lenta o fallida no retiene los resultados validos de las otras superficies; el aviso global enumera solo las cargas incompletas y no convierte sus fallos en un fallo general del dashboard. Entrar en Monitores conserva ademas su refresco propio de fuentes.

El escenario cerrado `pwa-bootstrap-isolation` usa API, Vite, autenticacion y PostgreSQL aislados, siembra un monitor inactivo y falla localmente cada lectura derivada inicial por separado. Antes del refresco propio de Monitores, el monitor debe existir ya en el selector de Oportunidades; despues debe seguir visible en su pestaña y el aviso debe identificar la superficie fallida. Worker y watchdog permanecen detenidos, todo host no loopback se bloquea y el usuario, sus sesiones y el monitor QA se eliminan al terminar.
