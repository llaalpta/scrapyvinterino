...
| `proxy_profiles` | Posee transporte, pais, disponibilidad, preset de locale/idioma/viewport/`x-screen`, plantilla sticky y TTL sticky `1..120`. Persiste un contador monotono y un fingerprint HMAC-SHA256 keyed por `APP_SECRET_KEY` sobre scheme/host/port/username/password, preset, plantilla y TTL; sesion/tarea conservan el token combinado `v1:<contador>:<digest>`. Manual y productor capturan ID+token. Antes del primer evento, el run toma un advisory lock transaccional compartido y revalida existencia, actividad, cooldown, pais/preset, contrato sticky y token; varias ejecuciones admitidas pueden compartirlo. Un perfil activo es de solo lectura salvo `Pausar`: el API decide sobre la fila bloqueada y rechaza incluso pausa+configuracion atomica. La edicion exige una pausa ya confirmada, permanece pausada al guardar y la reactivacion es otro comando. Pausa, edicion y reconciliacion toman el advisory exclusivo y `FOR NO KEY UPDATE`, por lo que ganan antes del fence y dejan cero construcciones/llamadas o esperan hasta el primer commit durable posterior a la ultima llamada del provider. El cierre `finalizing` posterior ya no conserva el lock porque no vuelve a emitir trafico. |
| `vinted_sessions` | Una fila conserva monitor, proxy, generacion efectiva, sticky ID, perfil, contexto geografico, estado, contadores y tiempos. Cookies, CSRF y tokens se serializan en `context_encrypted`; la generacion opaca, fingerprint de contexto, IP/pais, errores saneados y lifecycle metadata quedan fuera del cifrado. `expires_at` usa el menor plazo entre el TTL global del contexto anonimo y el TTL sticky del perfil. Una edicion de identidad, plantilla o TTL bloquea proxy y monitores afectados en orden estable e invalida/purga sus contextos. El read model devuelve como maximo una fila canonica por proxy y monitor. No hay una fila en Redis equivalente. |
...
| `CurlCffiVintedCatalogProvider` | En modo serial hay un solo provider/jar activo a la vez. La preparacion exporta el contexto y cierra su provider; la ejecucion reabre ese contexto aceptado sobre el mismo sticky para catalogo y detalles. Antes de aceptar candidatos, el run puede cerrar un primer intento recuperable y crear un unico reemplazo con sticky nuevo. Los modos explicitos canary/parallel clonan el contexto aceptado en hasta dos providers de lane con el mismo proxy/sticky, adoptan el ultimo contexto exitoso y canary vuelve a validar catalogo. El diagnostico de egress usa otra sesion sin cookies; el reemplazo fuerza uno no cacheado antes de construir su provider Vinted. |
| API/PWA/eventos | `Iniciar sesion` es la unica entrada PWA normal: prepara el contexto cuando hace falta y calibra antes de activar el monitor. Los endpoints directos de preparacion y detail probe permanecen como diagnosticos de desarrollo y crean runs de auditoria saneados, pero no se exponen como controles de producto. Monitores muestra la misma fila canonica que puede escoger el runtime, con `usable_now` y un motivo seguro; Ajustes se limita al estado operativo del proxy. La sesion local de usuario protege estas rutas, pero es independiente del contexto Vinted anonimo. |
...
sin candidata efectiva
  -> sticky nuevo -> bootstrap catalogo -> collector DataDome -> probe diagnostico
       ...
       -> fallo recuperable pre-candidatos   -> invalidar/cerrar intento
          -> un sticky nuevo + diagnostico real no cacheado
             -> misma IP conocida            -> rechazar antes de Vinted
             -> contexto/catalogo aceptado   -> continuar el mismo run
             -> segundo fallo                -> cooldown unico + run failed

ready -> seleccionar + incrementar uso -> cargar mismo jar/sticky -> catalogo/detalles
      -> rotacion detectada en detalle -> actualizar la misma fila y renovar el TTL efectivo
      -> rotacion ordinaria en catalogo/probe -> puede no persistirse (14.12.4)
      -> challenge/rechazo/transporte antes de candidatos -> mismo reemplazo unico anterior
      -> 429 de catalogo -> run failed inmediato + invalid + payload cifrado vacio
...
La recuperacion automatica permanece dentro del mismo run. Cada perfil recibe como maximo su intento inicial y un sticky nuevo; el reemplazo fuerza un diagnostico proxificado y una IP ya rechazada consume el intento sin construir otro cliente Vinted. Al agotarse A, su cooldown se confirma y su fence se libera antes de capturar una vez los demas perfiles elegibles. Cada traspaso A→B se serializa con la admision normal, comprueba capacidad SQL+Redis deduplicada por `task_id` y confirma en `runs.runtime_metadata` el perfil y generation de B antes de construir proveedor. B vuelve a validar identidad y capacidad bajo su fence; si cambian, no genera trafico y se considera el siguiente candidato capturado. Solo el agotamiento del pool termina como `session_acquisition_exhausted`. Un `429`, un fallo local de Redis/configuracion o cualquier challenge tras aceptar candidatos no entra en el loop. El consumer confirma la reserva una vez tras el unico terminal, sin escalada ni requeue.

Tras ese terminal, la PWA puede ofrecer un reintento explicito solo para los perfiles enfriados que constan como intentados en el ultimo baseline fallido. El endpoint autenticado vuelve a comprobar que el monitor sigue inactivo, el perfil sigue en cooldown y conserva configuracion, pais, identidad y capacidad validos; no borra la penalizacion. La admision de proxy/identidad precede al lock del monitor y el origen se revalida bajo ese lock. La orden fuerza un sticky y diagnostico nuevos, compara la salida contra un HMAC de la IP rechazada sin copiar esa IP a los metadatos de intento y ejecuta una sola vez sin recorrer el pool. En recurrentes, deployment y heartbeat siguen siendo obligatorios, pero la capacidad cuenta el perfil seleccionado pese a su cooldown. El exito abre el baseline/sesion normal y limpia solo ese perfil; el fallo conserva o amplifica su cooldown.
...
6. La API/worker resuelve la sesion Vinted canonicamente utilizable del monitor para el proxy residencial sticky seleccionado. Si no existe o no supera la elegibilidad efectiva, el intento inicial prepara una nueva; un ciphertext ilegible o contexto inutilizable puede consumir el reemplazo unico, nunca caer silenciosamente a otra fila o perfil.
7. Cada intento crea una sesion `curl_cffi` con `impersonate`. La preparacion navega el documento de catalogo, extrae contexto anonimo seguro, prueba la API con la misma sesion y guarda cookies/tokens cifrados solo si alcanza el punto de persistencia.
8. El intento inicial puede reutilizar el diagnostico reciente de su misma sesion/sticky. Un reemplazo fuerza siempre un diagnostico cookie-free a traves del sticky nuevo; pais/egress invalidos o la misma IP conocida detienen ese intento antes de Vinted.
9. Si el primer intento falla de forma recuperable antes de aceptar candidatos, se invalida y se intenta una sola vez con el sticky nuevo. Un segundo fallo enfria ese perfil una vez; el mismo run puede continuar por los restantes perfiles capturados, cada uno con el mismo limite, hasta exito o `session_acquisition_exhausted`.
10. Con el contexto anonimo aceptado se pide o reutiliza el payload del catalogo JSON. Desde que sus candidatos se aceptan, ninguna fase posterior reinicia el catalogo.
11. Se reclaman los candidatos nuevos deduplicados contra Redis; su cantidad es el unico `items_found` durable, mientras el tamano bruto del catalogo queda en eventos tecnicos. El baseline calibra con `items_found=0`. No existe una cola de detalle entre runs.
...
15. La sesion Vinted del monitor se conserva cifrada para usos posteriores hasta el menor deadline global/sticky, agotar contador, alcanzar el limite opcional de usos del monitor o invalidarse por rechazo/challenge. La sesion funcional del monitor permanece independiente y el siguiente run prepara contexto HTTP nuevo cuando sea necesario.
16. La PWA muestra oportunidades, estado de ejecucion y diagnosticos saneados de sesion en los logs del monitor.
17. `runs.task_id` permite reconocer una tarea redeliverada: los runs terminales no repiten trafico, los `finalizing` convergen Redis y un `running` huerfano se cierra antes del nuevo intento. Si ese run ya se habia traspasado, su perfil/generation durable gobierna la redelivery; el payload Redis conserva solo la admision inicial.
