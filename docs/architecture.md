...
La recuperacion automatica termina dentro del mismo perfil y run: antes de aceptar candidatos, el primer Cloudflare/DataDome, rechazo de sesion/contexto o fallo de transporte puede cerrar e invalidar su intento y usar un unico sticky nuevo. Ese reemplazo fuerza un diagnostico proxificado; una IP ya rechazada consume el intento sin construir otro cliente Vinted. Solo el agotamiento aplica una vez el cooldown y termina como `profile_session_acquisition_exhausted`. Un `429`, un fallo local de Redis/configuracion/identidad o cualquier challenge tras aceptar candidatos no entra en el loop. El consumer confirma la reserva una vez tras el unico terminal, sin escalada ni requeue. Un challenge de detalle descarta el lote reclamado y libera sus locks; las cookies que rotan en una respuesta ordinaria de detalle marcan el contexto para persistencia.
...
9. Si el primer intento falla de forma recuperable antes de aceptar candidatos, se invalida y se intenta una sola vez con el sticky nuevo. Un segundo fallo enfria ese perfil una vez y termina el mismo run; no hay fallback de perfil en 14.54.2.
