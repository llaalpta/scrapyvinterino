...
Status: `ready` for one final bounded retry after `ops/worker-proxy-dns-diagnostic`; neither bounded 2026-07-16 attempt passed all acceptance criteria. One intervening real PWA `Test IP` proved that base gateway HTTPS connectivity had recovered in the API container, but it did not prove country, sticky identity, DataDome, Vinted or worker-container readiness and waived none of this gate's checks. The later worker-boundary diagnostic now proves gateway resolution plus one sticky-proxy HTTPS request with valid ES context, without changing the acceptance itself. This is one acceptance outcome over the existing runtime, not a scheduler redesign. It uses one temporary continuous monitor with interval `60`, jitter `10%`, the existing live API/PWA/PostgreSQL/Redis, one eligible ES proxy, and the real Compose worker producer/consumers plus scheduler-watchdog. The first attempt left current worker/watchdog containers deliberately stopped; the retry confirmed their Compose actions with a dry run, started only those executors, waited for two advancing producer heartbeats before watchdog, and left the healthy API, Vite, PostgreSQL, Redis and frontend ownership unchanged during the representative scenario.

Before external traffic there must be no active monitor, non-terminal run, open monitor session or Redis key. The operational scheduler row is initially absent, so the gate first enables it through the PWA while the worker remains unavailable. One authenticated recurring `start` must then return the existing worker-unavailable rejection before a baseline, run, session, Redis mutation or provider call. The gate starts worker, waits for a fresh healthy heartbeat, starts watchdog and issues exactly one PWA `start`; acceptance requires that start to succeed.
