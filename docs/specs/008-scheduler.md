...
- Every non-archived monitor can be selected from the compact monitor table to show one performance block directly below its identity: accumulated monitor metrics first, explicitly including the active session, followed by the same business metrics for the active session or latest closed session. Configuration remains editable only while stopped and without a non-terminal run; the default all-history full-width bar chart of `items_found` remains accumulated and its range controls affect only that chart.
