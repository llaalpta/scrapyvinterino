...
Status: `blocked` after `qa/live-recurring-session-acceptance-retry`; neither bounded 2026-07-16 attempt passed all acceptance criteria. One intervening real PWA `Test IP` proved that base gateway HTTPS connectivity had recovered in the API container, but it did not prove country, sticky identity, DataDome, Vinted or worker-container readiness and waived none of this gate's checks. This is one acceptance outcome over the existing runtime, not a scheduler redesign. It uses one temporary continuous monitor with interval `60`, jitter `10%`, the existing live API/PWA/PostgreSQL/Redis, one eligible ES proxy, and the real Compose worker producer/consumers plus scheduler-watchdog. The first attempt left current worker/watchdog containers deliberately stopped; the retry confirmed their Compose actions with a dry run, started only those executors, waited for two advancing producer heartbeats before watchdog, and left the healthy API, Vite, PostgreSQL, Redis and frontend ownership unchanged during the representative scenario.
