...
- DataDome collector: automatic monitor session preparation may call the DataDome JS endpoint in the same `curl_cffi` session, sticky proxy identity, browser profile, locale, viewport and catalog document context. It attempts only the observed `ch` then `le` collector sequence. Missing `datadome` is logged as missing context, but during the current measurement phase it does not block the session when the catalog API probe returns accepted JSON.
- Catalog API probe: monitor session preparation calls `/api/v2/catalog/items` once after the catalog document bootstrap. The probe returns only redacted diagnostics, is stored as run evidence, and may mark a monitor-owned Vinted session usable only when the base context is complete and the API returns JSON. It never persists items, creates opportunities, or changes monitor metrics.
...
- Run events expose safe diagnostics: `browser_profile`, `impersonate`, `vinted_session_id`, `vinted_session_use_count`, `session_end_reason`, `recovery_action`, `proxy_session_id_prefix`, target/proxy country, locale, `Accept-Language`, viewport, `vinted_screen`, DataDome/CSRF/anon/access-token/v_udt presence booleans, probe outcome, rate-limit `Retry-After`/backoff fields, and masked/fingerprinted session markers only.
- DataDome collector diagnostics expose only method, URL, status, duration, `jsType`, `ddv`, DataDome key presence/length, JS payload length, attempt count and cookie presence flags. Raw cookies, tokens, `ddk`, `jspl`, collector response cookie values and full proxy credentials are never logged or returned.
