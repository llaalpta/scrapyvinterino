...
- DataDome collector: the explicit prepared-session preflight may call the DataDome JS endpoint in the same `curl_cffi` session, sticky proxy identity, browser profile, locale, viewport and catalog document context. It attempts only the observed `ch` then `le` collector sequence and treats missing `datadome` as an incomplete session; there is no browser/Playwright fallback and no catalog API fallback.
- Catalog API probe: the PWA may trigger an explicit proxy-only diagnostic that bootstraps the catalog document and calls `/api/v2/catalog/items` once even when `datadome` is missing. The probe returns only redacted diagnostics and never marks a Vinted session ready, persists items, creates runs, or changes monitor metrics.
...
- Prepared-session preflight attempts the DataDome collector only after the catalog document produced the base context needed for a conservative API request: CSRF, anon id, `access_token_web`, `v_udt`, target/egress country match, locale, `Accept-Language`, viewport and `x-screen=catalog`. If anything except `datadome` is missing, the collector is skipped and the session remains incomplete.
- The DataDome collector loads a returned `datadome` cookie into the same HTTP session before the prepared session is exported. If the collector returns no cookie, errors, or lacks the DataDome client key, the prepared session is persisted as incomplete and no Vinted catalog API request is attempted.
- The catalog API probe reports `accepted_json`, `challenge`, `rejected`, `non_json`, or `transport_error` with safe request/response metadata. Its result is evidence for future tuning only and does not relax the normal prepared-session gate.
