...
- Treat each configured monitor as the operational context for URL, cadence, optional filters, optional proxy, and runtime metadata.
...
- Fetch or reuse item detail for monitor-new candidates when filters require detail data.
...
- Store per-monitor seen state for speed and auditability.
...
- Historical reprocessing when filters change. Changing filters affects future runs only.
...
- API/PWA:
  ...
  - assign filters, cadence, and proxy profile on the monitor;
...
- A monitor stores its filters, proxy, cadence, and execution mode directly.
- Each monitor run fetches the configured fast catalog page and deduplicates candidates by `vinted_item_id`.
- An item already seen by the same monitor is skipped without creating another opportunity.
...
- Opportunity creation is idempotent for `source_id + item_id` in the monitor flow.
...
- Run the same item twice in the same monitor and confirm no duplicate opportunity or repeated filter work.
