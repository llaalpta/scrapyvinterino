...
const emptyProxyDraft: ProxyDraft = {
  ...
  port: '',
  maxConcurrentRuns: '1',
  stickyUsernameTemplate: '{username};sessid.{session_id}',
  stickyTtlMinutes: '25',
  username: '',
  password: '',
  countryCode: 'ES'
  ...
  async function onCreateProxy(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await createProxyProfile({
        ...
        sticky_username_template: proxyDraft.stickyUsernameTemplate,
        sticky_ttl_minutes: Number(proxyDraft.stickyTtlMinutes),
